...
class AuthHandler:
    """Handler class for authentication-related API endpoints."""
    
    def __init__(self, supabase_client=None, security_manager=None, notification_service=None, logger_instance=None):
    ...
    def register(self):
        ...
        try:
            ...
            existing_username = self.supabase.table('users').select('username').eq('username', username).execute()
            ...
            if hasattr(response, 'error') and response.error:
                ...
                self.logger.error(f"Supabase Auth error: {error_message}")
    ...
    def login(self):
        ...
        self.logger.info(f"SUPABASE_URL: {supabase_url}")
        self.logger.info(f"SUPABASE_KEY exists: {supabase_key_exists}")
        ...
        try:
            ...
            self.logger.info("Attempting Supabase auth...")
    ...
    def get_user_info(self):
        ...
        try:
            user_data = self.supabase.table('users').select('username, email, created_at').eq('id', current_user_uid).execute()
            if user_data.data:
    ...
    def update_username(self):
        ...
        try:
            existing = self.supabase.table('users').select('username').eq('username', new_username).execute()
            if existing.data:
                return jsonify({"error": "Username already exists"}), 400
            update_response = self.supabase.table('users').update({'username': new_username}).eq('id', user_id).execute()
    ...
    def change_password(self):
        ...
        try:
            user_row = self.supabase.table('users').select('email').eq('id', user_id).execute()
            if not user_row.data:
                return jsonify({"error": "User not found"}), 404
            email = user_row.data[0]['email']
            login_resp = self.supabase.auth.sign_in_with_password({"email": email, "password": current_password})
            if not login_resp.user:
                return jsonify({"error": "Current password is incorrect"}), 400
            update_resp = self.supabase.auth.update_user({"password": new_password})
            if update_resp.user:
    ...
    def forgot_password(self):
        ...
        try:
            user_row = self.supabase.table('users').select('id, username').eq('email', email).execute()
            if not user_row.data:
                return jsonify({'error': 'No account found with this email.'}), 404
            username = user_row.data[0]['username']
            resp = self.supabase.auth.reset_password_for_email(email)
    ...
    def request_otp(self):
        ...
        expires_at = datetime.now(self.ph_tz) + timedelta(minutes=5)
        expires_at_naive = expires_at.replace(tzinfo=None)
        self.supabase.table('password_reset_otps').delete().eq('email', email).execute()
        self.supabase.table('password_reset_otps').insert({
            'email': email,
            'otp': otp,
            'expires_at': expires_at_naive.isoformat()
        }).execute()
        try:
            self.notification_service.send_otp_email_gmail(email, otp, username=user_row.data[0]['username'])
        except Exception as e:
    ...
    def verify_otp(self):
        ...
        otp_row = self.supabase.table('password_reset_otps').select('*').eq('email', email).eq('otp', otp).execute()
        if not otp_row.data:
            return jsonify({'error': 'Invalid OTP.'}), 400
        otp_data = otp_row.data[0]
        now_naive = datetime.now(self.ph_tz).replace(tzinfo=None)
        expires_at_naive = datetime.fromisoformat(otp_data['expires_at']).replace(tzinfo=None)
        if now_naive > expires_at_naive:
            return jsonify({'error': 'OTP has expired.'}), 400
        try:
            user_row = self.supabase.table('users').select('id').eq('email', email).execute()
            if not user_row.data:
                return jsonify({'error': 'User not found.'}), 404
            update_resp = self.supabase.auth.admin.update_user_by_id(user_row.data[0]['id'], {"password": new_password})
...
@jwt_required()
def get_user_info():
    return auth_handler.get_user_info()

@auth_bp.route('/api/user/username', methods=['PUT'])
@jwt_required()
def update_username():
    return auth_handler.update_username()

@auth_bp.route('/api/user/password', methods=['PUT'])
@jwt_required()
def change_password():
    return auth_handler.change_password()

@auth_bp.route('/api/forgot-password', methods=['POST'])
def forgot_password():
    return auth_handler.forgot_password()

@auth_bp.route('/api/request-otp', methods=['POST'])
def request_otp():
    return auth_handler.request_otp()

@auth_bp.route('/api/verify-otp', methods=['POST'])
def verify_otp():
    return auth_handler.verify_otp()

@auth_bp.route('/api/verify-otp-only', methods=['POST'])
def verify_otp_only():
    return auth_handler.verify_otp_only()
