...
from ..core.storage import download_image_from_supabase, download_image_bytes_from_supabase, upload_json_to_supabase, list_files_in_supabase
from ..core.storage_manager import download_json_from_supabase as download_json_from_storage_manager, upload_json_to_supabase as upload_json_to_storage_manager
...
from ..services.state import get_custom_progress, set_custom_progress, get_jobs_store

heatmap_bp = Blueprint('heatmap', __name__)


class HeatmapHandler:
    """Handler class for heatmap-related API endpoints."""
    
    def __init__(self, logger_instance=None):
        """Initialize the heatmap handler.
    ...
    def get_heatmap_preview(self, job_id):
        """Get heatmap preview image."""
        job_folder = os.path.join(RESULTS_FOLDER, job_id)
    ...
    def _get_detections_logic(self, job_id, current_user=None):
        """Shared logic for getting detections."""
        self.logger.info(f"DEBUG: get_detections_logic called with job_id='{job_id}', current_user={current_user}")
        # Validate user ownership if user is provided
        if current_user is not None:
            self.logger.info(f"DEBUG: Validating user ownership for job_id='{job_id}', user='{current_user}'")
            with get_db_connection_context() as conn:
                cur = conn.cursor()
                try:
                    cur.execute('SELECT "user" FROM jobs WHERE job_id = %s', (job_id,))
                    job_row = cur.fetchone()
                    self.logger.info(f"DEBUG: Database query result: {job_row}")
                    if not job_row:
                        self.logger.error(f"DEBUG: Job {job_id} not found in database")
                    ...
                    self.logger.info(f"DEBUG: Job owner: '{job_user}', requesting user: '{current_user}'")
                    if job_user != current_user:
                        self.logger.error(f"DEBUG: User {current_user} attempted to access job {job_id} owned by {job_user}")
                        return jsonify({"error": "Unauthorized: Job not found or not authorized"}), 403
                    self.logger.info(f"DEBUG: User ownership validated successfully")
                except Exception as e:
                    self.logger.error(f"DEBUG: Error validating job ownership: {type(e).__name__}: {e}")
                    import traceback
                    self.logger.error(f"DEBUG: Traceback:\n{traceback.format_exc()}")
                    return jsonify({"error": "Error validating job ownership"}), 500
        else:
            self.logger.info(f"DEBUG: No user validation required (current_user is None)")
        
        # Load detections from Supabase
        self.logger.info(f"DEBUG: Loading detections for job_id='{job_id}'")
        detections, fps = load_detections(job_id)
        self.logger.info(f"DEBUG: load_detections returned - detections: {type(detections).__name__} (None={detections is None}), fps: {fps}")
        ...
        if detections is None:
            self.logger.warning(f"DEBUG: Detections file not found for job {job_id}, returning empty detections array")
            response_data = {"detections": [], "fps": fps or 30}  # Default fps to 30 if not found
            self.logger.info(f"DEBUG: Returning empty detections for job {job_id}")
            return jsonify(response_data), 200
        
        self.logger.info(f"DEBUG: Preparing response with {len(detections)} detections, fps={fps}")
        response_data = {"detections": detections, "fps": fps}
        self.logger.info(f"DEBUG: Response data keys: {list(response_data.keys())}, detections count: {len(detections)}, fps type: {type(fps).__name__}")
        return jsonify(response_data), 200
    
    def get_detections_from_json(self, job_id):
        """Get detections for a job."""
        current_user = get_jwt_identity()
        return self._get_detections_logic(job_id, current_user)
    
    def regenerate_detections(self, job_id):
        """Regenerate and upload detections.json for a completed job if missing."""
        current_user = get_jwt_identity()
        self.logger.info(f"DEBUG: regenerate_detections called for job_id='{job_id}', user='{current_user}'")
        ...
        with get_db_connection_context() as conn:
            ...
            try:
                ...
                if job_user != current_user:
                    self.logger.error(f"DEBUG: User {current_user} attempted to regenerate detections for job {job_id} owned by {job_user}")
                    return jsonify({"error": "Unauthorized"}), 403
                
                if job_status != 'completed':
                    self.logger.error(f"DEBUG: Job {job_id} is not completed (status: {job_status})")
                    return jsonify({"error": "Job must be completed to regenerate detections"}), 400
            except Exception as e:
                self.logger.error(f"DEBUG: Error validating job: {type(e).__name__}: {e}")
                import traceback
                self.logger.error(f"DEBUG: Traceback:\n{traceback.format_exc()}")
        ...
        if detections is not None and len(detections) > 0:
            self.logger.info(f"DEBUG: Detections already exist for job {job_id} ({len(detections)} detections)")
            return jsonify({
        ...
        if not input_video_name:
            self.logger.error(f"DEBUG: No input video name for job {job_id}")
            return jsonify({"error": "No input video found for this job"}), 404
        
        video_path = os.path.join(UPLOAD_FOLDER, job_id, input_video_name)
        self.logger.info(f"DEBUG: Checking for video at: {video_path}")
        
        # If video doesn't exist locally, try to download from Supabase
        if not os.path.exists(video_path):
            self.logger.info(f"DEBUG: Video not found locally, checking Supabase for {job_id}/{input_video_name}")
            try:
                # Try to download video from Supabase
                storage_manager = get_storage_manager()
                video_supabase_path = f"{job_id}/{input_video_name}"
                self.logger.info(f"DEBUG: Attempting to download video from Supabase: {video_supabase_path}")
                ...
                if video_bytes:
                    ...
                    with open(video_path, 'wb') as f:
                        f.write(video_bytes)
                    self.logger.info(f"DEBUG: Downloaded video from Supabase to {video_path} ({len(video_bytes)} bytes)")
                else:
                    self.logger.error(f"DEBUG: Video file not found in Supabase at {video_supabase_path}")
                    return jsonify({
                ...
                self.logger.error(f"DEBUG: Error downloading video from Supabase: {type(e).__name__}: {e}")
                import traceback
                self.logger.error(f"DEBUG: Traceback:\n{traceback.format_exc()}")
                return jsonify({
        ...
        try:
            self.logger.info(f"DEBUG: Starting detection regeneration for job {job_id}")
            ...
            try:
                ...
                self.logger.warning(f"DEBUG: Failed to clean up temp file {temp_output}: {e}")
            
            if not detections or len(detections) == 0:
                self.logger.warning(f"DEBUG: No detections generated for job {job_id}")
            ...
            self.logger.info(f"DEBUG: Successfully regenerated and uploaded {len(detections)} detections for job {job_id}")
            return jsonify({
            ...
            self.logger.error(f"DEBUG: Error regenerating detections for job {job_id}: {type(e).__name__}: {e}")
            import traceback
            self.logger.error(f"DEBUG: Traceback:\n{traceback.format_exc()}")
            return jsonify({"error": f"Failed to regenerate detections: {str(e)}"}), 500
    
    def _get_heatmap_image_logic(self, job_id):
        ...
        self.logger.info(f"Attempting to download heatmap image from Supabase: {supabase_path}")
        img_bytes = download_image_bytes_from_supabase(supabase_path)
        if img_bytes is None:
            self.logger.error(f"Heatmap image not found in Supabase: {supabase_path}")
            return jsonify({"error": "Result image file not found in Supabase"}), 404
        
        self.logger.info(f"Successfully downloaded heatmap image from Supabase: {supabase_path}")
        return Response(img_bytes, mimetype="image/jpeg")
    
    def get_heatmap_image(self, job_id):
        """Get heatmap result image."""
        return self._get_heatmap_image_logic(job_id)
    
    def export_heatmap_csv(self, job_id):
        """Export heatmap analysis as CSV."""
        ...
        try:
            ...
            if start_time is not None and end_time is not None:
                ...
                self.logger.info(f"Found files for CSV export: {files}")
                self.logger.info(f"Looking for timestamp={timestamp}, uuid={unique_id}, start_time={start_time}, end_time={end_time}")
                ...
                if timestamp and unique_id:
                    ...
                    self.logger.info(f"Looking for specific custom heatmap: {supabase_path}")
                else:
                    # Try to find most recent matching custom heatmap
                    prefix = f"{job_id}/custom_heatmap_{float(start_time):.1f}_{float(end_time):.1f}_"
                    self.logger.info(f"Searching for files with prefix: {prefix}")
                    matching_files = [f for f in files if f.startswith(prefix)]
                    self.logger.info(f"Matching custom heatmaps found: {matching_files}")
                    
                    if matching_files:
                        supabase_path = sorted(matching_files)[-1]  # Get most recent
                        self.logger.info(f"Using most recent custom heatmap: {supabase_path}")
                    else:
                        self.logger.error(f"No custom heatmap found matching time range {start_time:.1f}-{end_time:.1f}, available files: {files[:10]}")
                ...
                supabase_path = f"{job_id}/video_heatmap.jpg"
                self.logger.info(f"Using standard heatmap path: {supabase_path}")
    ...
    def export_heatmap_pdf(self, job_id):
        """Export heatmap analysis as PDF."""
        try:
            ...
            if start_time is not None and end_time is not None:
                ...
                # Try to find the custom heatmap
                files = list_files_in_supabase(f"{job_id}")
                self.logger.info(f"Found files for PDF export: {files}")
                ...
                if timestamp and unique_id:
                    ...
                    if matching_files:
                        ...
                        self.logger.error(f"No custom heatmap found matching time range {start_time:.1f}-{end_time:.1f}")
            ...
            try:
                ...
                self.logger.error(f"ReportLab import error: {str(e)}")
            ...
            self.logger.error(f"PDF export error for job {job_id}: {str(e)}")
            import traceback
            self.logger.error(f"PDF export traceback: {traceback.format_exc()}")
            return jsonify({'error': f'PDF generation failed: {str(e)}'}), 500
    
    def generate_custom_heatmap(self, job_id):
        """Generate custom heatmap for a time range."""
        try:
    ...
    def get_custom_analysis(self, job_id):
        ...
        try:
            ...
            if timestamp and unique_id:
                ...
                self.logger.info(f"Looking for specific file: {supabase_path}")
            else:
                # Try listing to find matching files
                files = list_files_in_supabase(f"{job_id}")
                self.logger.info(f"Found files in Supabase for {job_id}: {files}")
                
                # Match the exact naming convention with .1f precision
                prefix = f"{job_id}/custom_heatmap_{float(start):.1f}_{float(end):.1f}_"
                self.logger.info(f"Looking for files matching prefix: {prefix}")
                
                matching_files = [f for f in files if f.startswith(prefix)]
                self.logger.info(f"Matching files found: {matching_files}")
                
                if matching_files:
                    supabase_path = sorted(matching_files)[-1]  # Get most recent
                    self.logger.info(f"Selected most recent file: {supabase_path}")
                else:
                    self.logger.error(f"No matching custom heatmap found for prefix: {prefix}")
            ...
            self.logger.error(f"Error in get_custom_analysis: {e}")
            import traceback
            self.logger.error(f"Traceback: {traceback.format_exc()}")
            return jsonify({"error": str(e)}), 500
    
    def export_heatmap_jpg(self, job_id):
        """Export heatmap as JPG."""
        try:
            start_time = request.args.get('start_time', type=float)
            end_time = request.args.get('end_time', type=float)
            timestamp = request.args.get('timestamp')
            ...
            self.logger.error(f"Error in export_heatmap_jpg: {e}")
            return jsonify({"error": str(e)}), 500
    
    def get_custom_heatmap_image(self, job_id):
        """Get custom heatmap image."""
        start = request.args.get('start')
        end = request.args.get('end')
        timestamp = request.args.get('timestamp')
        unique_id = request.args.get('uuid')
        
        self.logger.info(f"[CustomHeatmapImage] Request for job_id={job_id}, start={start}, end={end}, timestamp={timestamp}, uuid={unique_id}")
        
        if not all([start, end]):
            self.logger.error(f"[CustomHeatmapImage] Missing parameters: start={start}, end={end}")
            return jsonify({"error": "Missing start or end parameters"}), 400
            
        # List all files in the job's folder to find the right one
        files = list_files_in_supabase(f"{job_id}")
        self.logger.info(f"[CustomHeatmapImage] Found {len(files)} files in job folder: {[f for f in files if 'custom_heatmap' in f]}")
        
        if timestamp and unique_id:
            # Try exact match first
            supabase_path = f"{job_id}/custom_heatmap_{float(start):.1f}_{float(end):.1f}_{timestamp}_{unique_id}.jpg"
            self.logger.info(f"[CustomHeatmapImage] Trying exact match: {supabase_path}")
        else:
            ...
            self.logger.info(f"[CustomHeatmapImage] Searching for files matching pattern: {job_id}/custom_heatmap_{float(start):.1f}_{float(end):.1f}_*, found {len(matching_files)}")
            if matching_files:
                supabase_path = sorted(matching_files)[-1]  # Get most recent
                self.logger.info(f"[CustomHeatmapImage] Using most recent: {supabase_path}")
            else:
                supabase_path = f"{job_id}/custom_heatmap_{float(start):.1f}_{float(end):.1f}.jpg"
                self.logger.warning(f"[CustomHeatmapImage] No matching files found, trying default path: {supabase_path}")
        
        self.logger.info(f"[CustomHeatmapImage] Attempting to download: {supabase_path}")
        img_bytes = download_image_bytes_from_supabase(supabase_path)
        
        if img_bytes is None:
            self.logger.error(f"[CustomHeatmapImage] Failed to download image from {supabase_path}")
            return jsonify({
        ...
        self.logger.info(f"[CustomHeatmapImage] Successfully downloaded image, size: {len(img_bytes)} bytes")
        ...
        self.logger.info(f"[CustomHeatmapImage] Returning image response with status 200")
        return response
    
    def get_custom_heatmap_progress(self, job_id):
        """Get custom heatmap generation progress."""
    ...
    def _get_heatmap_analysis_logic(self, job_id):
        ...
        if floorplan is None:
            ...
            self.logger.info(f"Floorplan not found locally at {floorplan_path}, trying Supabase...")
            try:
                ...
                if floorplan is None:
                    ...
                    self.logger.warning(f"Floorplan not found in Supabase either: {floorplan_supabase_path}. Using default dimensions.")
                    # Use the heatmap image dimensions as fallback
                    floorplan_height, floorplan_width = img_gray.shape[:2]
                    self.logger.info(f"Using heatmap dimensions as floorplan fallback: {floorplan_width}x{floorplan_height}")
                else:
                    self.logger.info(f"Successfully loaded floorplan from Supabase: {floorplan_supabase_path}")
            except Exception as e:
                self.logger.error(f"Error loading floorplan from Supabase: {e}")
        ...
        if not force_refresh:
            try:
                cached_analysis = download_json_from_storage_manager(cache_path)
                if cached_analysis:
                    ...
                    if cache_source == 'ai' and cached_analysis.get('recommendations_provider'):
                        self.logger.info(f"Using cached AI analysis for job {job_id} (source: {cached_analysis.get('recommendations_provider')})")
                    ...
                    if use_ai and cache_source == 'rule':
                        self.logger.info(f"Cache contains rule-based recommendations for job {job_id}, skipping cache to retry AI")
                    # If AI is disabled, rule-based cache is fine to use
                    elif not use_ai:
                        self.logger.info(f"Using cached analysis for job {job_id} (AI disabled)")
                        return jsonify(cached_analysis)
            except Exception as e:
                self.logger.warning(f"Could not load cached analysis for job {job_id}: {e}")
        
        # STEP 2: No valid cache found, run fresh analysis
        self.logger.info(f"Running fresh analysis for job {job_id}")
        detections, fps = load_detections(job_id)
        ...
        if use_ai:
            ...
            if analysis.get('recommendations_source') == 'ai' and analysis.get('recommendations_provider'):
                should_cache = True
                self.logger.info(f"AI recommendations generated for job {job_id}, caching for future use")
            else:
                should_cache = False
                self.logger.info(f"AI failed or quota exceeded for job {job_id}, using rule-based fallback (not caching to allow retry)")
        else:
            # If AI is disabled, always cache (rule-based is the expected result)
            should_cache = True
            self.logger.info(f"AI disabled for job {job_id}, caching rule-based recommendations")
        
        if should_cache:
            try:
                upload_json_to_storage_manager(analysis, cache_path)
                self.logger.info(f"Successfully cached analysis for job {job_id}")
            except Exception as e:
                self.logger.warning(f"Could not cache analysis for job {job_id}: {e}")
                # Continue anyway - caching is optional
        
        return jsonify(analysis)
    
    def get_heatmap_analysis(self, job_id):
        """Get heatmap analysis for a job."""
        return self._get_heatmap_analysis_logic(job_id)


# Create handler instance
heatmap_handler = HeatmapHandler()

# Register routes
@heatmap_bp.route('/heatmap_jobs/test', methods=['GET', 'OPTIONS'])
@cross_origin()
def test_heatmap_cors():
    return heatmap_handler.test_heatmap_cors()

@heatmap_bp.route('/test', methods=['GET', 'OPTIONS'])
@cross_origin()
def test_direct_cors():
    return heatmap_handler.test_direct_cors()

@heatmap_bp.route('/heatmap_jobs/<job_id>/preview/detections', methods=['GET'])
@cross_origin()
def get_detection_preview(job_id):
    return heatmap_handler.get_detection_preview(job_id)

@heatmap_bp.route('/heatmap_jobs/<job_id>/preview/heatmap', methods=['GET'])
@cross_origin()
def get_heatmap_preview(job_id):
    return heatmap_handler.get_heatmap_preview(job_id)
