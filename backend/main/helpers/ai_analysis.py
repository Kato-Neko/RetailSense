...
class AIAnalysisService:
    """Service for generating AI-powered recommendations from heatmap data."""
    
    def __init__(self):
        """Initialize the AI analysis service."""
        self.provider_functions = {
            'groq': self._call_groq,
            'gemini': self._call_gemini_with_fallback,
            'openai': self._call_openai,
        }
        self.stream_provider_functions = {
            'groq': self._call_groq_stream,
            'gemini': self._call_gemini_stream,
            'openai': self._call_openai_stream,
        }
    
    def _get_ai_provider(self, provider: Optional[str] = None) -> Optional[str]:
        """
    ...
    def generate_ai_recommendations(
        ...
        Args:
            areas: Dictionary with traffic distribution areas
            total_visitors: Total number of unique visitors
            peak_hours: List of peak hour dictionaries
            provider: Optional provider name ('groq', 'gemini', 'openai')
            
        Returns:
            Tuple of (recommendations list, used_ai bool)
        """
        selected_provider = self._get_ai_provider(provider)
        ...
        provider_func = self.provider_functions.get(selected_provider)
        if provider_func:
            try:
    ...
    def generate_ai_recommendations_stream(
        ...
        selected_provider = self._get_ai_provider(provider)
        if not selected_provider:
            for rec in self._generate_rule_based_recommendations(areas, total_visitors):
                yield rec
            return
        ...
        stream_func = self.stream_provider_functions.get(selected_provider)
        if stream_func:
            try:
    ...
    def _call_gemini_with_fallback(self, prompt: str, context: Dict) -> List[str]:
        """Call Gemini API with model fallback support."""
    ...
    def _call_groq_stream(self, prompt: str):
        ...
        try:
            ...
            yield from self._parse_ai_stream(response_stream)
        except ImportError:
            raise RuntimeError("groq not installed")
    ...
    def _parse_ai_stream(self, response_stream):
...
def generate_ai_recommendations(
    ...
    return get_ai_analysis_service().generate_ai_recommendations(areas, total_visitors, peak_hours, provider)
...
def generate_ai_recommendations_stream(
    ...
    """Legacy function for backward compatibility."""
    return get_ai_analysis_service().generate_ai_recommendations_stream(areas, total_visitors, peak_hours, provider)
