...
#     
#     print("=== END TEST ===")
class HeatmapBlendingService:
    """Encapsulates heatmap generation and blending operations."""
    ...
    def blend_heatmap(self, detections, floorplan_path, output_heatmap_path, output_video_path, video_path, points=None, progress_callback=None, return_image=False):
        """Generate and blend heatmap; also create annotated video output."""
        ...
        try:
            self.create_progressive_heatmap_video_local(detections, floorplan, job_id, video_path, points)
    ...
    def create_progressive_heatmap_video_local(self, detections, floorplan, job_id, video_path, points=None):
        ...
        try:
            ...
            while True:
                ...
                heatmap_processed = np.power(heat_accum, 0.6)
                heat_smoothed = gaussian_filter(heatmap_processed, sigma=10)
...
_heatmap_service_singleton = HeatmapBlendingService()


def create_custom_heatmap(detections, floorplan_path, dimensions=(1920, 1080), points=None):
    return _heatmap_service_singleton.create_custom_heatmap(detections, floorplan_path, dimensions, points)
...
def blend_heatmap(detections, floorplan_path, output_heatmap_path, output_video_path, video_path, points=None, progress_callback=None, return_image=False):
    return _heatmap_service_singleton.blend_heatmap(
        detections,
        floorplan_path,
        output_heatmap_path,
        output_video_path,
...
def create_progressive_heatmap_video_local(detections, floorplan, job_id, video_path, points=None):
    return _heatmap_service_singleton.create_progressive_heatmap_video_local(detections, floorplan, job_id, video_path, points)