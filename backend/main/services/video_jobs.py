...
class VideoJobProcessorService:
    """Coordinates detection, heatmap generation and DB updates for video jobs."""
...
_video_job_service_singleton = VideoJobProcessorService()


def update_job_status_in_db(job_id: str, job: Dict[str, Any]):
    return _video_job_service_singleton.update_job_status_in_db(job_id, job)


def update_job_progress(job_id: str, stage: str, progress: float):
    return _video_job_service_singleton.update_job_progress(job_id, stage, progress)


def process_video_job(job_id: str):
    return _video_job_service_singleton.process_video_job(job_id)


def run_custom_heatmap_job(job_id: str, start_time: float, end_time: float, set_progress: Callable[[float], None]):
    return _video_job_service_singleton.run_custom_heatmap_job(job_id, start_time, end_time, set_progress)
