...
class TrackingService:
    """Service for person detection and tracking using YOLOv8 and DeepSort."""
    ...
    def _get_model(self):
        ...
        if self._model is None:
            ...
            self._model.model.eval()  # Set to evaluation mode
            torch.set_num_threads(1)  # Use single thread for better performance on small instances
            
            self.logger.info("YOLO model loaded and optimized for CPU inference")
        return self._model
    
    def _get_tracker(self):
        """Get or initialize the DeepSort tracker (lazy loading)."""
    ...
    def detect_and_track(
        ...
        Args:
            ...
            cancelled_flag: Optional callable that returns True if processing should be cancelled
            
        Returns:
            Tuple of (output_video_path, detections_for_heatmap, fps)
        """
        ...
        self.logger.info(f"Successfully opened video file: {video_path}")
        ...
        self.logger.info(f"Processing video: {original_width}x{original_height} -> {width}x{height} (scale: {scale_factor:.2f})")
        ...
        if progress_callback:
            ...
            self.logger.info(f"Starting video processing: {total_frames} total frames, will process {total_processed_frames} frames (every {frame_skip}th frame)")
        
        self.logger.info(f"Video properties: {original_width}x{original_height}, {fps} fps, {total_frames} total frames")
        ...
        try:
            while cap.isOpened():
                ...
                if frame is None:
                    self.logger.error(f"Frame {frame_count} is None, skipping")
                    frame_count += 1
                    continue
                ...
                if should_process:
                    ...
                    if not warmup_done:
                        self.logger.info("Warming up YOLO model with first frame...")
                        try:
                    ...
                    if processed_frames <= 3:
                        self.logger.info(f"Processing frame {frame_count + 1} (processed frame {processed_frames}), shape: {frame.shape}")
                    ...
                    try:
                        ...
                        self.logger.error(f"Error processing frame {frame_count} with YOLO: {e}")
                        frame_count += 1
                        continue
                    
                    yolo_time = time.time() - start_time
                    if processed_frames <= 3:
                        self.logger.info(f"YOLO inference took {yolo_time:.2f} seconds for frame {frame_count + 1}")
                    ...
                    if processed_frames <= 3:
                        self.logger.info(f"YOLO found {total_detections} total detections, {len(detections)} above threshold in processed frame {processed_frames}")

                    # Update tracks
                    try:
                        tracks = tracker.update_tracks(detections, frame=frame_resized)
                    except Exception as e:
                        self.logger.error(f"Error updating tracks for frame {frame_count}: {e}")
                        tracks = []
        ...
        self.logger.info(f"Video processing completed: {frame_count} frames read, {processed_frames} processed")
        return output_path, detections_for_heatmap, fps


# Global instance (shared across calls for model/tracker reuse)
...
def detect_and_track(
    ...
    return get_tracking_service().detect_and_track(video_path, output_path, progress_callback, preview_folder, cancelled_flag)
